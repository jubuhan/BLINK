...
const Navbar = () => {
  ...
  return (
    <div id="top" className='bg-white duration-200 z-40 sticky w-full font-Raleway shadow-md'>
      <div className='py-4'>
        <div className='px-6 flex justify-between items-center gap-4'>
          ...
          <div className='flex items-center justify-end'>
            {isLoggedIn ? (
              <button onClick={handleLogout} className='w-[100px] text-white bg-black/95 rounded-full px-3 py-2 hover:scale-105 transition duration-300 hover:shadow-lg hover:bg-white hover:text-black'>
                Logout
              </button>
            ) : (
              <button onClick={togglePop} className='w-[100px] text-white bg-black/95 rounded-full px-3 py-2 hover:scale-105 transition duration-300 hover:shadow-lg hover:bg-white hover:text-black'>
                Login
              </button>
            )}
          </div>
      ...
      {hamClick && <div className='bg-black/80 w-full h-screen fixed z-10 top-0 left-0'></div>}
